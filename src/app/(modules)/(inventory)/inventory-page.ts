...
export class InventoryComponent implements OnInit {
  ...
  filtered = computed(() => {
    ...
    if (q) list = list.filter((i) => i.itemName.toLowerCase().includes(q) || (i.skuCode ?? '').toLowerCase().includes(q));
    return list;
  });
  ...
  private load(): void {
    ...
    forkJoin({
      ...
      warehouses: this.warehouseApi.getAll(),
    }).subscribe({
      ...
      error: () => { this.loading.set(false); this.error('Failed to load inventory'); },
    });
  }
  ...
  save(): void {
    ...
    request$.subscribe({
      ...
      error: () => { this.saving.set(false); this.error('Failed to save item'); },
    });
  }
    ...
    this.confirm.confirm({
      ...
      accept:  () =>
        this.itemApi.delete(item.id).subscribe({
          next:  () => { this.items.update((list) => list.filter((x) => x.id !== item.id)); this.success('Item deleted'); },
